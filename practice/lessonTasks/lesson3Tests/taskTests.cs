...
namespace lesson3Tests 
{
    class taskTests //Доп проект и класс для тестирования методов классов из lesson3Tasks
    {
        ...
        {
            if (expected.nominator == actual.nominator && expected.denominator == actual.denominator && expected.intPart == 0)
            {
                Console.ForegroundColor = ConsoleColor.Green;
                Console.WriteLine("SUCCESS");
            }
            else if (expected.intPart != 0 && expected.intPart == actual.intPart || 
                (expected.nominator == actual.nominator && expected.denominator == actual.denominator))
            {
                Console.ForegroundColor = ConsoleColor.Green;
                Console.WriteLine("SUCCESS");
            }
